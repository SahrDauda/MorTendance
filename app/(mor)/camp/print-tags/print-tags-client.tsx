...
export function PrintTagsClient() {
  ...
  const filteredMembers = useMemo(() => {
    return members.filter((m) => {
      ...
      if (branchFilter !== "ALL") {
        const mBranch =
          (m.branch || "HQ").toLowerCase() === "headquarters"
            ? "HQ"
            : m.branch || "HQ"
        const targetBranch =
          branchFilter.toLowerCase() === "headquarters" ? "HQ" : branchFilter
        if (mBranch !== targetBranch) return false
      }
  ...
  }, [members, groupFilter, branchFilter, roleFilter])
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 bg-card p-6 rounded-2xl border shadow-sm">
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              Batch export or print standard 54mm × 76mm ID cards for MOR Camp 2026 delegates.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 print:grid-cols-2 print:gap-4 print:p-2">
            {filteredMembers.map((member) => (
              <div key={member.id} className="flex flex-col items-center gap-1.5 break-inside-avoid">
                <MorTagFront member={member} width="54mm" height="76.12mm" compact={true} />
              </div>
            ))}
          </div>
        )}
