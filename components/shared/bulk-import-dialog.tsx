...
interface BulkImportDialogProps {
    ...
    sampleData?: any[]
    parseMode?: "standard" | "raw-all-sheets"
}
...
export function BulkImportDialog({
        ...
        reader.onload = (event) => {
            try {
                ...
                if (parseMode === "raw-all-sheets") {
                    ...
                    setParsedData([allSheetsData]) // Wrap in array to satisfy state type loosely
                } else {
    ...
    const handleImport = async () => {
        ...
        setIsImporting(true)
        try {
            const dataToPass = parseMode === "raw-all-sheets" && parsedData ? parsedData[0] : parsedData
            const result = await onImport(dataToPass)
            if (result.success) {
            ...
            <DialogContent className="sm:max-w-[500px]">
                ...
                <div className="space-y-6 py-4">
                    ...
                    <div className="space-y-4">
                        <p className="text-sm font-semibold">{parseMode === "standard" ? "Step 2: Upload & Preview" : "Upload File"}</p>
                        <div
                            className={cn(
                        ...
                        >
                            ...
                            <div className="text-center">
                                ...
                                <p className="text-xs text-muted-foreground mt-1">
                                    {parsedData ? (parseMode === "raw-all-sheets" ? `${Object.keys(parsedData[0]).length} sheets found` : `${parsedData.length} rows found`) : "CSV or Excel files only"}
                                </p>
                            </div>
                        </div>
                    </div>
